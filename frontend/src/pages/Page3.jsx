...
import api, { getSongsByPlaylist } from '../utils/axios'
import bg1 from '../assets/page4bg.jpeg'
import bg2 from '../assets/page4bgM.jpeg'
...
const Page3 = () => {
  ...
  useEffect(()=>{
    const fetchSongs = async()=>{
      try{
        const data = await getSongsByPlaylist('heart-break')
          setSongs(data.songs)
  ...
  return (
    <div className='w-screen h-screen overflow-hidden relative'>
      ...
      <div
        ...
        className='absolute w-full h-full bg-cover bg-center bg-no-repeat inset-0 md:block'>
      </div>
      ...
      {/* Arrow to go back */}
      <div className='flex text-white absolute top-15 left-4 md:left-20 text-2xl md:text-5xl w-20 h-10 md:w-20 md:h-20 bg-gray-500/40 justify-center items-center rounded-full
              shadow-lg cursor-pointer hover:bg-gray-500/50 hover:scale-120 hover:shadow-2xl'>
      ...
      <div className='flex flex-col text-5xl md:text-7xl lg:text-7xl gap-2 md:gap-5 text-white/90 font-extrabold absolute 
       right-[4%] md:right-[8%] lg:right-[10%] top-[7%] md:top-[10%]'>
        ...
        <h1 className='drop-shadow-[0_4px_8px_rgba(0,0,0,0.8)]'>2 AM</h1>
